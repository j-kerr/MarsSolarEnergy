...
for wave in wavelengths:
    input_loc1 = datapath + 'integrated_flux_' + str(wave) + '_' + str(panel_angle) + 'degrees' + '_azimuth_' + str(panel_direction)
...
print(allwavelengths)
hm = sns.heatmap(allwavelengths, norm=LogNorm(), cmap='viridis', xticklabels = 6, yticklabels = 3, cbar_kws={'label': 'Solar Energy Ratio (Logarithmic Scale)'})
hm = hm.invert_yaxis()
hm = plt.xlabel('Solar Longitude ($^\circ$)')
hm = plt.ylabel('Latitude ($^\circ$)')
hm = plt.title('Solar Energy Gain for an Optimally Placed Static Panel Over a Horizontal Panel')
