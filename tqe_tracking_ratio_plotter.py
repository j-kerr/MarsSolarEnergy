...
for a in latitudes:
    for b in solar_longs:
        df_wavsplit = df_all2[(df_all2['latitude'] == a) & (df_all2['solar_longitude'] == b)]
        waven = df_wavsplit['flux'].sum()
        waven = waven * 1.6E-19 * 88775 #convert back to energy/day
        df_wavsum = pd.DataFrame([{'latitude':a, 'solar_longitude':b, 'flux':waven}])
...
print(allwavelengths)
hm = sns.heatmap(allwavelengths, norm=LogNorm(), cmap='viridis', xticklabels = 6, yticklabels = 3, cbar_kws={'label': 'Energy per Sol Ratio (Logarthmic Scale)'})
hm = hm.invert_yaxis()
