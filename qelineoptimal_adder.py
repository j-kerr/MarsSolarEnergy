...
for panel_angle in panel_angle_list:
    for panel_direction in panel_direction_list:
        ...
        df_optimal = pd.concat([df_optimal, allwavelengths])
for a in latitudes:
    df_line = df_optimal[(df_optimal['latitude'] == a)]
    df_0 = df_line[df_line['tilt'] == 0]
    zerosum = df_0['flux'].sum()
    angsum = zerosum
    for panel_angle in panel_angle_list:
        azsum = zerosum
        for panel_direction in panel_direction_list:
            df_line = df_optimal[(df_optimal['latitude'] == a) & (df_optimal['pointing'] == panel_direction) & (df_optimal['tilt'] == panel_angle)]
            isum = df_line['flux'].sum()
            if isum > azsum:
                azsum = isum
                optaz = panel_direction
        if azsum > angsum:
            angsum = azsum
            optang = panel_angle
    print('Latitude ' + str(a) + ' : Optimal angle is ' + str(optang) + ' and optimal azimuth is ' + str(optaz) + ' with total flux ' + str(azsum))
...
hm = sns.heatmap(df_plot, cmap='mako', xticklabels = 6, yticklabels = 3, cbar_kws={'label': 'Energy per Sol ($10^6$J)'})
hm = hm.invert_yaxis()
